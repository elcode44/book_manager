...
def search_and_display():
    rows = search(search_entry.get())
    output.delete("1.0", "end")  # clear previous results
    for row in rows:
...
searcher = tk.Button(root, text="Search", command=search_and_display)
searcher.pack(anchor="w", padx=10)
...
book_name.pack(anchor = "w", padx=10, pady =5)

# author name
author_frame = tk.Frame(root)
author_frame.pack(anchor = "w", padx = 20, pady=5)
...
length_frame.pack(anchor = "w", padx = 20, pady=5)

length_label = tk.Label(root, text = "Book length")
length_label.pack(anchor = "w", padx = 10)

length_entry = tk.Entry(root)
length_entry.pack(anchor = "w", padx=10, pady =5)

# book genre
genre_frame = tk.Frame(root)
genre_frame.pack(anchor = "w", padx = 20, pady=5)

genre_label = tk.Label(root, text = "Book genre")
genre_label.pack(anchor = "w", padx = 10)

genre_entry = tk.Entry(root)
genre_entry.pack(anchor = "w", padx=10, pady =5)

#add button
add_frame = tk.Frame(root)
add_frame.pack(anchor = "w", padx = 20, pady=5)

add_a_book = tk.Button(root, text = "Add book", command = lambda: (add_one(book_name.get(), author_entry.get(), length_entry.get(), genre_entry.get()), update()))
add_a_book.pack(anchor = "w", padx = 10)

# delete book button
delete_frame = tk.Frame(root)
delete_frame.pack(anchor = "w", padx = 10, pady=5)

delete_label = tk.Label(root, text = "name of book to delete")
delete_label.pack(anchor = "w", padx = 10)

delete_entry = tk.Entry(root)
delete_entry.pack(anchor = "w", padx=10, pady =5)

#add button
delete_frame = tk.Frame(root)
delete_frame.pack(anchor = "w", padx = 20, pady=5)

delete_a_book = tk.Button(root, text = "Delete a book", command = lambda: (delete_one(delete_entry.get()), update()))
...
#starts with the books already on screen
update()
